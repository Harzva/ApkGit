...
`v0.2.3` 继续把 Android experimental APK 往发布级打磨：移动端头部改成更薄的产品操作区，底部导航收敛到 5 个主任务，搜索、来源、发现结果和下载统计改成更稳的单列触控流；CI 改用稳定 experimental 签名，后续版本可以覆盖安装。Android 仍只产出一个包：`gitmarket-android-experimental.apk`。
...
| Rust / egui 客户端 | `0.2.3` | 桌面发现页并发搜索、懒加载结果、降低空白感和重绘开销；移动端头部、导航、搜索、来源筛选和卡片流继续升级 |
| Android Rust experimental APK | `0.2.3` | 唯一 Android 发布包；使用公开 experimental 测试签名以保持后续覆盖安装能力，不再额外维护 WebView preview APK |
