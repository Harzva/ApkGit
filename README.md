...
`v0.3.7` 在 `npx` 启动器和新版 iOS SwiftUI 预览基础上，新增 App 版本中心：Home 直接显示当前版本、在线版本和最新下载入口；在线版本来自公开 JSON manifest，后续每个 App 都可以复用同一套“检查版本 -> 下载最新 -> 按平台覆盖安装”的产品能力。CI 继续产出 Android APK、iOS simulator、Apple Silicon / Intel macOS、Windows 和 Linux 包。Android 仍只产出一个包：`gitmarket-android-experimental.apk`。
...
| 在线版本 JSON | [app-update.json](https://harzva.github.io/GitReleaseMarket/app-update.json) |
...
```bash
GITMARKET_VERSION=v0.3.7 npx github:Harzva/GitReleaseMarket
```
...
| Rust / egui 客户端 | `0.3.7` | Release 发现保持主线，Skill / MCP 广场作为设置中手动开启的能力插件；移动端补齐中文字体兜底和窄屏卡片布局 |
| npx GitHub 启动器 | `0.3.7` | `npx github:Harzva/GitReleaseMarket` 会下载并运行当前平台的 GitHub Release 桌面包 |
| App 更新 manifest | `0.3.7` | `docs/app-update.json` 暴露最新版本、Release URL、平台下载地址和覆盖安装策略 |
| Android Rust experimental APK | `0.3.7` | 唯一 Android 发布包；使用公开 experimental 测试签名以保持后续覆盖安装能力，不再额外维护 WebView preview APK |
...
| App Skills Suite | 初始可用 | `skills/app-skills-suite` 固化版本中心、在线 manifest、一键下载和覆盖安装约束 |
